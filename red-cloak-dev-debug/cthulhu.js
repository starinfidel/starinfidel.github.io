...
Console.prototype = {
	...
	,runCommandCommon: function(cmd,tokens) {
		if(cmd == "help" || cmd == "h" || cmd == "?") {
			...
			return this.runDebugCommand(tokens);
	...
	,runDebugCommand: function(tokens) {
		if(tokens.length == 0) {
			this._console.print("<span class=consoleSys>" + ("Debug commands:\n" + "anxiety (a) - set anxiety in chat\n" + "eval (e) - toggle always on evaluate timer\n" + "rapport (r) - set rapport in chat\n" + "skill (sk) [id/name] [val] - set skill value") + "</span>");
			return 1;
		}
		var cmd = tokens[0];
		if(cmd == "anxiety" || cmd == "a") {
			if(this.game.get_state() != _$GameState.STATE_CHAT) {
				this._console.print("<span class=consoleSys>" + "Not in chat." + "</span>");
				return 1;
			}
			var val = Std.parseInt(tokens[1]);
			this._console.print("<span class=consoleSys>" + ("[Anxiety " + val + "]") + "</span>");
			this.game.npc.set_anxiety(val);
			return 1;
		} else if(cmd == "eval" || cmd == "e") {
			this.game.debug.evaluate = !this.game.debug.evaluate;
			this._console.print("<span class=consoleSys>" + ("[Evaluate always on: " + Std.string(this.game.debug.evaluate) + "]") + "</span>");
			return 1;
			...
			var val1 = Std.parseInt(tokens[1]);
			this._console.print("<span class=consoleSys>" + ("[Rapport " + val1 + "]") + "</span>");
			this.game.npc.set_rapport(val1);
			return 1;
		} else if(cmd == "skill" || cmd == "sk") {
			var id = tokens[1];
			var val2 = Std.parseInt(tokens[2]);
			var skill = SkillConst.getInfo(id);
			if(skill == null) {
				this._console.print("<span class=consoleSys>" + "No such skill." + "</span>");
				return 0;
			}
			var key = skill.id;
			var _this = this.game.player.skills;
			var playerSkill = __map_reserved[key] != null ? _this.getReserved(key) : _this.h[key];
			playerSkill.val = val2;
			this._console.print("<span class=consoleSys>" + ("[Skill " + skill.name + " set to " + val2 + ".]") + "</span>");
			return 1;
		}
		return 0;
	}
	...
	,debug: function(s) {
		...
		_this.output.append("<span class=\"consoleDebug\">" + s + "</span><br>");
		_this.console.scrollTop(_this.inner.height());
	}
	,error: function(s) {
...
ConsoleJS.prototype = {
	...
	,debug: function(s) {
		this.output.append("<span class=\"consoleDebug\">" + s + "</span><br>");
		this.console.scrollTop(this.inner.height());
	}
};
var Const = function() { };
...
Game.prototype = {
	...
	,set_state: function(st) {
		...
		_this.output.append("<span class=\"consoleDebug\">" + ("Game state: " + Std.string(this._state)) + "</span><br>");
		_this.console.scrollTop(_this.inner.height());
		return st;
	}
};
...
NPC.prototype = {
	...
	,rollCommand: function(info,playerSkill) {
		...
		if(!(!this.chatSkills.get(id).isEnabled)) {
			...
			_this1.output.append("<span class=\"consoleDebug\">" + ("disabled " + id) + "</span><br>");
			_this1.console.scrollTop(_this1.inner.height());
	...
	,set_anxiety: function(v) {
		...
		_this.output.append("<span class=\"consoleDebug\">" + ("anxiety " + this._anxiety + " " + (mod > 0 ? "+" : "") + mod + " = " + v) + "</span><br>");
		_this.console.scrollTop(_this.inner.height());
		this._anxiety = v;
		return v;
	}
	...
	,enableSkill: function(id) {
		...
		var _this1 = this.game.console._console;
		_this1.output.append("<span class=\"consoleDebug\">" + ("enabled " + id) + "</span><br>");
		_this1.console.scrollTop(_this1.inner.height());
		var _this2 = this.chatSkills;
	...
	,set_rapport: function(v) {
		...
		var _this = this.game.console._console;
		_this.output.append("<span class=\"consoleDebug\">" + ("rapport " + this._rapport + " " + (mod > 0 ? "+" : "") + mod + " = " + v) + "</span><br>");
		_this.console.scrollTop(_this.inner.height());
		this._rapport = v;
		return v;
	}
	...
	,set_chatState: function(v) {
		...
		var _this = this.game.console._console;
		_this.output.append("<span class=\"consoleDebug\">" + ("set state to " + Std.string(v)) + "</span><br>");
		_this.console.scrollTop(_this.inner.height());
		return v;
	}
};
	...
	var value4 = { id : "redCloak", isEnabled : true, func : function() {
		if(_gthis.get_chatState() != ChatState.NPC_STATE_ENMITY) {
			...
			if(!(!_gthis.chatSpecialTopics.get("redCloak").isEnabled)) {
				...
				_this1.output.append("<span class=\"consoleDebug\">" + ("disabled " + "redCloak") + "</span><br>");
				_this1.console.scrollTop(_this1.inner.height());
...
infos_ButlerThorston.prototype = $extend(NPC.prototype,{
	startChatPre: function() {
		...
		if(!this.chatSpecialTopics.get("redCloak").isEnabled) {
			var _this = this.game.console._console;
			_this.output.append("<span class=\"consoleDebug\">" + ("enabled " + "redCloak") + "</span><br>");
			_this.console.scrollTop(_this.inner.height());
			this.chatSpecialTopics.get("redCloak").isEnabled = true;
		}
		if(!this.chatSkills.get("law").isEnabled) {
			var _this1 = this.game.console._console;
			_this1.output.append("<span class=\"consoleDebug\">" + ("enabled " + "law") + "</span><br>");
			_this1.console.scrollTop(_this1.inner.height());
			this.chatSkills.get("law").isEnabled = true;
		}
		if(!this.chatSkills.get("credit").isEnabled) {
			var _this2 = this.game.console._console;
			_this2.output.append("<span class=\"consoleDebug\">" + ("enabled " + "credit") + "</span><br>");
			_this2.console.scrollTop(_this2.inner.height());
