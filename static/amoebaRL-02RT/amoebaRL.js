...
function repaintScreen()
{
  ...
  var now = new Date();
  var ms = now.valueOf() - playerStart.valueOf();
  ...
  status.innerHTML =
    ...
    '<td>' +
    '<p style="color:white; text-align:right;">Time: ' + time + '</p>' +
    '<td halign=right>' +
...
function gameFinished()
{
  ...
  if (gameState == 1)
    screen.innerHTML =
      '<div style="margin-top:180; color:#ff0000"><center>' +
      '<b>You lose. Press SPACE to restart.</b></center></div>';
  else
    {
      var now = new Date();
      var ms = now.valueOf() - playerStart.valueOf();
      var playerTime = parseInt(ms / 1000.0);

      var bestTime = getCookie('amoebaRL.bestTime');

      // reset record
      if (playerTime < bestTime || bestTime == null)
        setCookie('amoebaRL.bestTime', playerTime);
      ...
      screen.innerHTML =
        ...
        '<b>You win in ' + playerTime + ' seconds. ' +
	(playerTime < bestTime ?
	  "You set a new record!" :
	  "Best record: " + bestTime + ' seconds.') +
