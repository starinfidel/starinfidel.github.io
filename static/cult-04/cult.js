...
CustomMenu = function(uivar,gvar) { if( uivar === $_ ) return; {
	...
	var y = 10;
	{
		...
		while(_g < _g1.length) {
			...
			var el = null;
			if(info.type == "bool") el = Tools.checkbox({ id : info.name, text : "" + Reflect.field(Static.difficulty[2],info.name), w : 70, h : 20, x : 320, y : y, fontSize : 14, container : divel});
			else el = Tools.textfield({ id : info.name, text : "" + Reflect.field(Static.difficulty[2],info.name), w : 70, h : 20, x : 320, y : y, fontSize : 14, container : divel});
