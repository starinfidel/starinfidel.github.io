...
UI.prototype.onMapMove = function(event) {
	...
	if(cell != null && cell.isVisible) this.tip(cell.getNote());
	else this.tip("");
	this.prevX = x;
	this.prevY = y;
}
...
Cell.prototype.paint = function(screen,isSelected,rect) {
	...
	else if(this.map.hasMarker(this.x,this.y)) {
...
Creature.prototype.aiAlertAroundMe = function() {
	var _g1 = (this.y - 3), _g = (this.y + 3);
	while(_g1 < _g) {
		var yy = _g1++;
		var _g3 = (this.x - 3), _g2 = (this.x + 3);
		while(_g3 < _g2) {
			var xx = _g3++;
...
Reanimated.prototype.ai = function() {
	...
	if(this.level > 1) {
		...
		if(c1 != null) {
			this.ui.msg("The reanimated hungrily devours the body.");
...
Reanimated.prototype.hit = function(who) {
	...
	this.die();
	this.ui.msg("Reanimated has been put down.");
}
...
Map.prototype.generateCreatures = function() {
	var cnt = Std["int"]((this.width * this.height) / 17);
	{
		var _g = 0;
		while(_g < cnt) {
			var i = _g++;
...
Reflect.hasField = function(o,field) {
	...
	{ var $it13 = arr.iterator();
	while( $it13.hasNext() ) { var t = $it13.next();
...
Cop.prototype.ai = function() {
	...
	if(c != null) {
		this.ui.msg("Cop attacks the reanimated.");
		c.object.hit(this);
		return;
	}
...
Body.prototype.activate = function(player) {
	...
	if(nx == -1) {
		this.ui.msg("Your laboratory is full!");
		return false;
	}
	this.move(nx,ny);
	this.ui.msg("You bring the specimen to your laboratory.");
	this.game.panic += 10;
	this.map.paint();
	this.ui.paintStatus();
	return false;
...
Body.prototype.testSolution = function(player) {
	...
	if(100 * Math.random() < 45 - 15 * this.quality) {
		this.ui.msg("You test a new solution on the body... Nothing happens.");
		return true;
	}
	var mod = 0;
	...
	if(100 * Math.random() < 70) {
		...
		if(cell == null) {
			haxe.Log.trace("no empty spots near lab",{ fileName : "Body.hx", lineNumber : 57, className : "Body", methodName : "testSolution"});
			return true;
		}
	...
	this.ui.msg(((spawnOk?"With the new solution the body is reanimated! ":"You have failed to reanimate the body. ")) + ((mod > 0?("[Theory +" + mod) + "]":"")));
	return true;
}
