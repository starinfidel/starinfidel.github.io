...
Log = function(uivar,gvar) { if( uivar === $_ ) return; {
	this.ui = uivar;
	this.game = gvar;
	this.window = Tools.window({ id : "window", center : true, winW : UI.winWidth, winH : UI.winHeight, fontSize : 18, w : 800, h : 500, z : 20});
...
Cult.prototype.declareWar = function(cult) {
	...
	this.ui.log(((this.getFullName() + " has declared war against ") + cult.getFullName()) + ".");
}
...
Cult.prototype.lowerWillpower = function(pwr) {
	...
	if(100 * Math.random() < 30 * this.difficulty.investigatorWillpower) {
		if(!this.isAI) {
...
Cult.prototype.makePeace = function(cult) {
	...
	this.ui.log(((this.getFullName() + " has made peace with ") + cult.getFullName()) + ".");
}
...
Game.prototype.endTimer = function(name) {
	if(Game.debugTime) haxe.Log.trace(((name + ": ") + (Date.now().getTime() - this.timerTime)) + "ms",{ fileName : "Game.hx", lineNumber : 257, className : "Game", methodName : "endTimer"});
}
	...
	}
	{
		...
		while(_g < _g1.length) {
			...
			if(c.isAI && !c.isDead) {
				...
				c.aiTurn();
				if(Game.debugTime) haxe.Log.trace(((("ai " + c.name) + ": ") + (Date.now().getTime() - this.timerTime)) + "ms",{ fileName : "Game.hx", lineNumber : 257, className : "Game", methodName : "endTimer"});
			}
		}
	}
	this.player.turn();
...
Game.prototype.restart = function(newDifficulty) {
	...
	this.ui.updateStatus();
	if(Game.debugTime) haxe.Log.trace((("restart" + ": ") + (Date.now().getTime() - this.timerTime)) + "ms",{ fileName : "Game.hx", lineNumber : 257, className : "Game", methodName : "endTimer"});
}
...
Debug.prototype.addItem = function(title,func) {
	this.lastMenuY += 30;
	Tools.button({ id : "menuItem" + this.lastMenuY, fontSize : 14, bold : false, text : title, w : 200, h : 22, x : 10, y : this.lastMenuY, container : this.menu, func : func});
}
...
Debug.prototype.onAI = function(event) {
	...
	haxe.Log.trace("trace ai " + ((Game.debugAI?"on":"off")),{ fileName : "Debug.hx", lineNumber : 98, className : "Debug", methodName : "onAI"});
}
...
Debug.prototype.onGiveAdepts = function(event) {
	this.onGivePower(null);
	{
		...
		while(_g < 3) {
			var i = _g++;
			{ var $it9 = this.game.player.nodes.iterator();
			while( $it9.hasNext() ) { var n = $it9.next();
			{
				if(n.level < 1 && Math.random() < 0.5) n.upgrade();
				{ var $it10 = n.links.iterator();
				while( $it10.hasNext() ) { var n2 = $it10.next();
...
Debug.prototype.onNear = function(event) {
	...
	haxe.Log.trace("node nearness info " + ((Game.debugNear?"on":"off")),{ fileName : "Debug.hx", lineNumber : 118, className : "Debug", methodName : "onNear"});
}
...
Debug.prototype.onTiming = function(event) {
	...
	haxe.Log.trace("timing " + ((Game.debugTime?"on":"off")),{ fileName : "Debug.hx", lineNumber : 90, className : "Debug", methodName : "onTiming"});
}
Debug.prototype.onToggleInvisible = function(event) {
	this.game.player.isDebugInvisible = !this.game.player.isDebugInvisible;
	haxe.Log.trace("invisibility " + ((this.game.player.isDebugInvisible?"on":"off")),{ fileName : "Debug.hx", lineNumber : 126, className : "Debug", methodName : "onToggleInvisible"});
}
...
Debug.prototype.onVis = function(event) {
	...
	haxe.Log.trace("node visibility to cults info " + ((Game.debugVis?"on":"off")),{ fileName : "Debug.hx", lineNumber : 108, className : "Debug", methodName : "onVis"});
}
...
AI.prototype.aiLowerAwareness = function() {
	if((this.awareness < 10 && !this.hasInvestigator) || (this.awareness < 5 && this.hasInvestigator) || this.getAdepts() == 0 || this.adeptsUsed >= this.getAdepts()) return;
	...
	if(Game.debugAI && this.awareness != prevAwareness) haxe.Log.trace(((((this.name + " awareness ") + prevAwareness) + "% -> ") + this.awareness) + "%",{ fileName : "AI.hx", lineNumber : 287, className : "AI", methodName : "aiLowerAwareness"});
}
AI.prototype.aiLowerAwarenessHard = function() {
	...
	if(Game.debugAI && this.awareness != prevAwareness) haxe.Log.trace(((((this.name + " virgin awareness ") + prevAwareness) + "% -> ") + this.awareness) + "%",{ fileName : "AI.hx", lineNumber : 268, className : "AI", methodName : "aiLowerAwarenessHard"});
}
AI.prototype.aiLowerWillpower = function() {
	if(!this.hasInvestigator || (this.awareness < 5 && !this.isRitual) || this.investigator.isHidden || this.getAdepts() == 0) return;
	{
		var _g1 = 0, _g = Game.numPowers;
		while(_g1 < _g) {
			var i = _g1++;
...
AI.prototype.aiSummon = function() {
	...
	if(Game.debugAI) haxe.Log.trace(this.name + " TRY SUMMON!",{ fileName : "AI.hx", lineNumber : 298, className : "AI", methodName : "aiSummon"});
	this.summonStart();
}
...
AI.prototype.aiUpgradeFollowers = function() {
	...
	if(this.getAdepts() < 5 && this.getUpgradeChance(0) > 70 && this.getVirgins() > 0) {
		while(true) {
			...
			this.upgrade(0);
			if(Game.debugAI) haxe.Log.trace((this.name + " upgrade neophyte, adepts: ") + this.getAdepts(),{ fileName : "AI.hx", lineNumber : 213, className : "AI", methodName : "aiUpgradeFollowers"});
		}
		return;
	}
	if(this.getPriests() < 3 && this.getUpgradeChance(1) > 60 && this.getVirgins() > 1) {
		while(true) {
			...
			this.upgrade(1);
			if(Game.debugAI) haxe.Log.trace((("!!! " + this.name) + " upgrade adept, priests: ") + this.getPriests(),{ fileName : "AI.hx", lineNumber : 229, className : "AI", methodName : "aiUpgradeFollowers"});
		}
		return;
	}
}
...
Node.prototype.clearLines = function() {
	...
	{ var $it11 = this.lines.iterator();
	while( $it11.hasNext() ) { var l = $it11.next();
	{
		l.clear();
...
Node.prototype.paintLines = function() {
	...
	var dist = 10000;
	var nc = null;
	{ var $it13 = this.owner.nodes.iterator();
...
Node.prototype.updateLinkVisibility = function(cult) {
	{ var $it19 = this.links.iterator();
	while( $it19.hasNext() ) { var n = $it19.next();
	if(n.visibility[cult.id] && n.owner != cult) {
		var vis = false;
		{ var $it20 = n.links.iterator();
		while( $it20.hasNext() ) { var n2 = $it20.next();
		if(n2.owner == cult) {
			vis = true;
			break;
		}
...
Alert = function(uivar,gvar) { if( uivar === $_ ) return; {
	this.ui = uivar;
	this.game = gvar;
	this.window = Tools.window({ id : "window", center : true, winW : UI.winWidth, winH : UI.winHeight, bold : true, w : 600, h : 450, z : 20});
	this.window.style.visibility = "hidden";
...
Alert.prototype.show = function(s) {
...
Investigator.prototype.killFollower = function() {
	...
	var node = null;
	if(this.cult.isRitual) { var $it23 = this.cult.nodes.iterator();
	while( $it23.hasNext() ) { var n = $it23.next();
	{
	...
	else { var $it24 = this.cult.nodes.iterator();
...
Investigator.prototype.turn = function() {
	...
	if(this.isHidden && this.numTurn > this.cult.difficulty.investigatorTurnVisible) {
		this.ui.log(this.cult.getFullName() + " has found out the investigator's location.");
		this.isHidden = false;
	}
...
MainMenu = function(uivar,gvar) { if( uivar === $_ ) return; {
	this.ui = uivar;
	this.game = gvar;
	this.window = Tools.window({ id : "mainMenuWindow", center : true, winW : UI.winWidth, winH : UI.winHeight, w : 420, h : 200, z : 20});
	...
	this.bg = js.Lib.document.createElement("div");
	this.bg.style.visibility = "hidden";
	this.bg.style.position = "absolute";
	this.bg.style.zIndex = 15;
	this.bg.style.width = UI.winWidth + 20;
	this.bg.style.height = UI.winHeight;
	this.bg.style.left = 0;
	this.bg.style.top = 0;
	this.bg.style.opacity = 0.8;
	this.bg.style.background = "#000";
	js.Lib.document.body.appendChild(this.bg);
	var close = Tools.closeButton(this.window,160,160,"mainMenuClose");
	close.onclick = $closure(this,"onClose");
}}
...
Status.tipEndTurn = "Click to end current turn.";
Status.tipInfo = "Click to view cults information.";
Status.tipMainMenu = "Click to open main menu.";
Status.tipLog = "Click to view message log.";
