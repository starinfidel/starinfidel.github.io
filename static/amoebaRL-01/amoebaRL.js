// amoebaRL by darkinfidel@gmail.com
...
function repaintScreen()
{
  ...
  status.innerHTML =
    ...
    '<td halign=right>' +
    '<p style="text-align:right;margin-right:5">Games total: ' + gamesTotal +
    '&nbsp;&nbsp;Won: ' + gamesWon +
...
//  window.status = event.charCode;
  if (event.keyCode == 38 || event.charCode == 56)
    playerY--;

  // down
  else if (event.keyCode == 40 || event.charCode == 50)
    playerY++;

  // left
  else if (event.keyCode == 37 || event.charCode == 52)
    playerX--;

  // right
  else if (event.keyCode == 39 || event.charCode == 54)
    playerX++;

  // ul
  else if (event.keyCode == 36 || event.charCode == 55)
    {
      playerX--;
      playerY--;
    }
  ...
  // ur
  else if (event.keyCode == 33 || event.charCode == 57)
    {
      playerX++;
      playerY--;
    }
  ...
  // dl
  else if (event.keyCode == 35 || event.charCode == 49)
    {
      playerX--;
      playerY++;
    }
  ...
  // dr
  else if (event.keyCode == 34 || event.charCode == 51)
    {
      playerX++;
      playerY++;
    }
...
function gameFinished()
{
  ...
  if (gameState == 1)
    ...
    screen.innerHTML =
      '<div style="margin-top:180;"><center><b>You win. Press SPACE to restart.</b></center></div>';
}
