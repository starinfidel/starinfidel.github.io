...
Cult.prototype.removeCloseGenerators = function() {
	...
	{
		{ var $it4 = n.links.iterator();
		while( $it4.hasNext() ) { var n2 = $it4.next();
		if(n2.owner == null && n2.isGenerator) n2.setGenerator(false);
		}}
...
UINode.prototype.update = function() {
	...
	if(Game.debugNear) {
		...
		{ var $it9 = this.node.links.iterator();
...
Game.prototype.endTurn = function() {
	{
		...
		while(_g < _g1.length) {
			...
			if(c.isAI && !c.isDead) {
				...
				c.aiTurn();
				if(Game.debugTime) haxe.Log.trace(((("ai " + c.name) + ": ") + (Date.now().getTime() - this.timerTime)) + "ms",{ fileName : "Game.hx", lineNumber : 252, className : "Game", methodName : "endTimer"});
			}
		}
	}
	this.player.turn();
...
Debug.prototype.onAI = function(event) {
	...
	haxe.Log.trace("trace ai " + ((Game.debugAI?"on":"off")),{ fileName : "Debug.hx", lineNumber : 81, className : "Debug", methodName : "onAI"});
}
...
Debug.prototype.onNear = function(event) {
	...
	haxe.Log.trace("node nearness info " + ((Game.debugNear?"on":"off")),{ fileName : "Debug.hx", lineNumber : 101, className : "Debug", methodName : "onNear"});
}
...
Debug.prototype.onTiming = function(event) {
	...
	haxe.Log.trace("timing " + ((Game.debugTime?"on":"off")),{ fileName : "Debug.hx", lineNumber : 73, className : "Debug", methodName : "onTiming"});
}
Debug.prototype.onToggleInvisible = function(event) {
	this.game.player.isDebugInvisible = !this.game.player.isDebugInvisible;
	haxe.Log.trace("invisibility " + ((this.game.player.isDebugInvisible?"on":"off")),{ fileName : "Debug.hx", lineNumber : 109, className : "Debug", methodName : "onToggleInvisible"});
}
...
Debug.prototype.onVis = function(event) {
	...
	haxe.Log.trace("node visibility to cults info " + ((Game.debugVis?"on":"off")),{ fileName : "Debug.hx", lineNumber : 91, className : "Debug", methodName : "onVis"});
}
...
Node.prototype.setVisible = function(cult,v) {
	...
	if(!cult.isAI) {
		if(Game.mapVisible) v = true;
		{ var $it15 = this.lines.iterator();
		while( $it15.hasNext() ) { var l = $it15.next();
		l.setVisible(v);
		}}
	}
}
...
Node.prototype.updateLinkVisibility = function(cult) {
	...
	if(n.visibility[cult.id] && n.owner != cult) {
		var vis = false;
		{ var $it19 = n.links.iterator();
		while( $it19.hasNext() ) { var n2 = $it19.next();
		if(n2.owner == cult) {
			vis = true;
			break;
		}
...
Investigator.prototype.killFollower = function() {
	...
	var node = null;
	if(this.cult.isRitual) { var $it22 = this.cult.nodes.iterator();
	...
	else { var $it23 = this.cult.nodes.iterator();
...
Status.tipFollowers = ["Neophytes can find some virgins if they're lucky.","Adepts can lower society awareness.",("3 priests and " + Game.numSummonVirgins) + " virgins are needed to summon the Elder God."];
...
Status.tipLowerWillpower = "Your adepts can use resources to lower investigator willpower.<br>Cost: ";
